...
SD_Error bsp_SD_Init(void)
{
	...
	MY_NVIC_Init(1, 0, SDIO_IRQn, 1);        //SDIO中断配置
...
SD_Error SD_ReadBlock(u8 *buf, long long addr, u16 blksize)
{
	...
	if (DeviceMode == SD_POLLING_MODE)                        //查询模式,轮询数据
	{
		...
		while (((DMA2->ISR & 0X2000) == 0) && (TransferEnd == 0) && (TransferError == SD_OK) && timeout)
			timeout--;//等待传输完成
...
SD_Error SD_ReadMultiBlocks(u8 *buf, long long addr, u16 blksize, u32 nblks)
{
	...
	if (nblks > 1)                                            //多块读
	{
		...
		if (DeviceMode == SD_POLLING_MODE) {
			...
			SDIO->MASK |= (1 << 1) | (1 << 3) | (1 << 8) | (1 << 5) | (1 << 9);    //配置需要的中断
			SDIO->DCTRL |= 1 << 3;                                //SDIO DMA使能
			while (((DMA2->ISR & 0X2000) == 0) && timeout)timeout--;//等待传输完成
			if (timeout == 0)return SD_DATA_TIMEOUT;//超时
...
SD_Error SD_WriteBlock(u8 *buf, long long addr, u16 blksize)
{
	...
	if (DeviceMode == SD_POLLING_MODE) {
		...
		while (((DMA2->ISR & 0X2000) == 0) && timeout)timeout--;//等待传输完成
		if (timeout == 0) {
			bsp_SD_Init();                        //重新初始化SD卡,可以解决写入死机的问题
...
SD_Error SD_ProcessIRQSrc(void)
{
	if (SDIO->STA & (1 << 8))//接收完成中断
	{
		...
		SDIO->MASK &= ~((1 << 1) | (1 << 3) | (1 << 8) | (1 << 14) | (1 << 15) | (1 << 4) | (1 << 5) |
						(1 << 9));//关闭相关中断
		TransferEnd = 1;
